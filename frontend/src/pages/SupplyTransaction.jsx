...
const SupplyTransaction = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      const response = await axios.get(`git@github.com:MEET2231/REDALERT_FIRESTATIONMANAGEMENTSYSTEM.git/api/suppliers?sortByTotalItems=true&order=${sortOrder}`);
  ...
  const fetchItems = async () => {
    try {
      const response = await axios.get('git@github.com:MEET2231/REDALERT_FIRESTATIONMANAGEMENTSYSTEM.git/api/items');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        await axios.put(`git@github.com:MEET2231/REDALERT_FIRESTATIONMANAGEMENTSYSTEM.git/api/suppliers/${supplierId}/items/${itemId}`, {
          Price: formData.Price
        });
        setMessage("✅ Supply transaction updated successfully!");
      } else {
        await axios.post(`git@github.com:MEET2231/REDALERT_FIRESTATIONMANAGEMENTSYSTEM.git/api/suppliers/${formData.Supplier_ID}/items`, {
  ...
  const handleDelete = async (supplierId, itemId) => {
    if (window.confirm('Are you sure you want to delete this supply transaction?')) {
      try {
        await axios.delete(`git@github.com:MEET2231/REDALERT_FIRESTATIONMANAGEMENTSYSTEM.git/api/suppliers/${supplierId}/items/${itemId}`);
